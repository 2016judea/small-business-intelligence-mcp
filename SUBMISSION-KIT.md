...
2. **Business verification — DONE 2026-08-31**, under Brick and Mortar AI LLC. It
   ...
   org that holds the verification. This was the only thing blocking the filing;
   everything else had been ready since 08-30.
...
7. **The form imports a JSON.** `chatgpt-app-submission.json` in this repo fills
   App Info, MCP Server and Testing in one upload. It is generated by OpenAI's
   own `chatgpt-app-submission` skill, whose definition is public at
   `github.com/openai/plugins/blob/main/plugins/openai-developers/skills/chatgpt-app-submission/SKILL.md`
   — read it and run its workflow directly; Codex is not installed here and is
   not needed. Regenerate it whenever a tool is added, and verify the result
   against a live `tools/list` so the file and Scan Tools cannot disagree.
8. **Skills are a separate upload and a separate scan.** `skills/` holds three
   `SKILL.md` files in OpenAI's own layout (`skills/<name>/SKILL.md`, frontmatter
   `name` / `description` / `metadata.short-description`); zip that directory and
   upload it. The safety scan is advertised as up to 2 hours and took well under
   it. **The scan reads the skill text only** — it exercises no dataset, so a pass
   is not a verdict on anything the tools return.
9. **Starter prompts and release notes** are §9 above. Both appear publicly.
10. Approval does not publish it. Publishing is a second, separate click in the
